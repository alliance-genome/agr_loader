...
class Neo4jHelper(object):
    ...
    def create_indices():
        ...
        session.run("CREATE INDEX ON :FBBTTerm(primaryKey)")
        ...
        session.run("CREATE INDEX ON :PATOTerm(primaryKey)")
        session.run("CREATE INDEX ON :APOTerm(primaryKey)")
        session.run("CREATE INDEX ON :DPOTerm(primaryKey)")
        session.run("CREATE INDEX ON :FYPOTerm(primaryKey)")
        session.run("CREATE INDEX ON :WBPhenotypeTerm(primaryKey)")
        session.run("CREATE INDEX ON :MPTerm(primaryKey)")
        session.run("CREATE INDEX ON :HPTerm(primaryKey)")
