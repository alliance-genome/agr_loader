...
class ResourceDescriptorService(object):
    ...
    def alter_prefixes_to_match_resource_yaml(entry):
        ...
        prefix_translation_dictionary = {'WORMBASE': 'WB',
                                         'FLYBASE': 'FB',
                                         'RCSB PDB': 'RCSB_PDB'}

        if entry in prefix_translation_dictionary:
    ...
    def return_url(self, identifier, page):
        ...
        if regex_output is None:
            self.logger.info('Fatal Error: Cross Reference identifier did not match Resource Descriptor YAML file gid pattern.')
        ...
        if page is None and default_url is not None:
            ...
            self.logger.info('Fatal Error: Cross Reference page is specified as None but the default url is not specified in the YAML.')
            self.logger.info('Database prefix: %s', db_prefix)
            ...
            try:
                ...
                self.logger.info('Fatal Error: The specified Cross Reference page or database prefix does not appear to exist.')
                self.logger.info('Database prefix: %s', db_prefix)
