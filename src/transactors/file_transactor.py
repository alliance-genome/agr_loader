...
class FileTransactor():
    ...
    def shutdown(self):
        ...
        FileTransactor.logger.debug("Shutting down FileTransactor threads: %s",
                                    len(self.thread_pool))
        for thread in self.thread_pool:
            thread.terminate()
        FileTransactor.logger.debug("Finished Shutting down FileTransactor threads")


    def run(self, filetracking_queue):
        '''Run'''

        FileTransactor.logger.debug("%s: Starting FileTransactor Thread Runner.",
                                    self._get_name())
        while True:
            try:
                (sub_type, FileTransactor.count) = FileTransactor.queue.get()
            except EOFError as error:
                FileTransactor.logger.debug("Queue Closed exiting: %s", error)
                return

            FileTransactor.logger.debug("%s: Pulled File Transaction Batch: %s QueueSize: %s ",
                                        self._get_name(),
                                        FileTransactor.count,
                                        FileTransactor.queue.qsize())
